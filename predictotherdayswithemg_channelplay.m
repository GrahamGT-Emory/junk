...
%%
emgsig = Y1;
emgsig = emgsig(:,6);  %selecting just one channel
figure, plot(emgsig)
forcesig1 = X1;
[Xr,Hinv] = LSoracle(forcesig1,emgsig); 
figure
subplot(4,2,2), plot(Xr(:,1), Xr(:,2), '*'), title('Oracle Prediction of force from EMG'), xlabel('force X'), ylabel('force Y')% gives really cool reaching graph     diff units?
set(gca,'xtick',[])
set(gca,'ytick',[])
